...
export function NewMaintenanceRequestModal({ open, onOpenChange }: { open: boolean; onOpenChange: (open: boolean) => void }) {
  const { role, currentUser, locations, addMaintenanceRequest } = useApp()
  ...
  const handleSubmit = (event: FormEvent<HTMLFormElement>) => {
    ...
    if (!locationId || !area || !category || !title.trim() || !description.trim()) return
          ...
          <div className="grid gap-4 px-5 py-5 sm:grid-cols-2">
            <div className="space-y-1.5">
              ...
              <div className="relative">
                ...
                <select id="maintenance-location" className={`${fieldClass} pl-8`} value={locationId} onChange={(event) => setLocationId(event.target.value)} disabled={role !== "owner"} required>
            ...
            <div className="space-y-1.5">
              ...
              <select id="maintenance-area" className={fieldClass} value={area} onChange={(event) => setArea(event.target.value)} required>
            ...
            <div className="space-y-1.5">
              ...
              <select id="maintenance-category" className={fieldClass} value={category} onChange={(event) => setCategory(event.target.value)} required>
            ...
            <div className="space-y-1.5 sm:col-span-2">
              ...
              <Input id="maintenance-title" value={title} onChange={(event) => setTitle(event.target.value)} placeholder="Example: Preschool classroom AC not cooling" required />
            </div>

            <div className="space-y-1.5 sm:col-span-2">
              <Label htmlFor="maintenance-description">Description</Label>
              <Textarea id="maintenance-description" value={description} onChange={(event) => setDescription(event.target.value)} rows={4} placeholder="Describe what happened, any temporary safety measures, and what needs attention." required />
            </div>

            <div className="space-y-1.5">
              <Label htmlFor="maintenance-date">Date Reported</Label>
              <Input id="maintenance-date" type="date" value={dateReported} onChange={(event) => setDateReported(event.target.value)} required />
            </div>
            ...
            <div className="space-y-1.5">
              ...
              <div className="relative">
                ...
                <Input id="maintenance-cost" className="pl-6" type="number" min="0" step="0.01" value={estimatedCost} onChange={(event) => setEstimatedCost(event.target.value)} placeholder="0.00" />
              </div>
            </div>

            <div className="space-y-1.5 sm:col-span-2">
              <Label htmlFor="maintenance-photo">Photo upload <span className="font-normal text-muted-foreground">(prototype)</span></Label>
              <label htmlFor="maintenance-photo" className="flex min-h-20 cursor-pointer items-center gap-3 rounded-lg border border-dashed border-border bg-muted/25 px-4 py-3 transition-colors hover:border-primary/30 hover:bg-muted/45">
                ...
                <span className="min-w-0 text-sm">
                  ...
                  <span className="text-xs text-muted-foreground">The filename is stored locally for this prototype.</span>
                </span>
              </label>
              <Input id="maintenance-photo" className="sr-only" type="file" accept="image/*" onChange={(event) => setPhotoName(event.target.files?.[0]?.name ?? "")} />
            </div>

            <label className="flex cursor-pointer items-start gap-3 rounded-lg border border-border bg-muted/25 p-3 sm:col-span-2">
              <input type="checkbox" className="mt-0.5 h-4 w-4 accent-primary" checked={approvalRequired} onChange={(event) => setApprovalRequired(event.target.checked)} />
              <span>
            ...
            <div className="rounded-lg bg-muted/40 px-3 py-2.5 sm:col-span-2">
              ...
              <p className="mt-0.5 text-sm font-medium text-foreground">{currentUser.name} · {currentUser.role.replace("_", " ")}</p>
            </div>
          </div>
